...
## Времязатраты
Во время выполнения задания обязательно фиксируйте времязатраты по каждому отдельному пункту и выписывайте их в отдельном файле (`TIMETRACKING.md` в корне проекта). Фиксировать нужно время затраченное и на изучение проекта, и на изучение сути задания, и на итоговую реализацию.

## Порядок работы
1. Данный github проект необходимо форкнуть и все последующие действия выполнять только в своем форке
2. По окончании работы отправить ссылку на свой форк с созданным и заполненным файлом `TIMETRACKING.md` на адрес `bazzy.bazzy@gmail.com`
...
1. По адресу `/flights` выводится таблица всех существующих в БД рейсов, с использованием плагина `datatables`. Данные загружаются с использованием ajax. Необходимо сделать сортировку и фильтрацию в данной таблице. И то и то должно происходить на стороне сервера. Сортировка по полям: `дата вылета`, `время вылета`, `время прилета`, `наличие мест`.
